...
Note that the there are 15 unused bits in each entry.
...
Jump : 10 (5632 bytes)</td>
<td class="highlight-yellow">34 (Offset = 0)</td>
<td class="highlight-yellow">86 (Offset = 512)</td></tr>
<tr>
<td class="highlight-yellow" style="text-align: right;">124 (Offset = 1024) </td>
<td class="highlight-yellow">516 (Offset = 1536)</td>
<td class="highlight-yellow">166 (Offset = 2560)</td></tr>
<tr>
<td class="highlight-yellow" style="text-align: right;">180 (Offset = 3072)</td>
<td class="highlight-yellow">1048 (Offset = 3584)</td>
<td class="highlight-yellow">201 (Offset = 5120)</td></tr>
<tr>
<td class="highlight-green" style="text-align: right;">Jump : 12 (6656 bytes)</td>
<td class="highlight-green">2001 (Offset = 5632)</td>
<td class="highlight-green">55 (Offset = 7680)</td></tr>
<tr>
<td class="highlight-green" style="text-align: right;">789 (Offset = 8192)</td>
<td class="highlight-green">221 (Offset = 9216)</td>
<td class="highlight-green">12* (Offset = 9728)</td></tr>
<tr>
<td class="highlight-green" style="text-align: right;">68 (Offset = 10240)</td>
<td class="highlight-green">514 (Offset = 11274)</td>
<td class="highlight-green">18 (Offset = 11776)</td></tr>
<tr>
<td class="highlight-blue" style="text-align: right;">6166 (Offset = 12288)</td>
<td class="highlight-blue">364 (Offset = 18944)</td>
<td class="highlight-blue">44 (Offset = 19456)</td></tr>
<tr>
<td class="highlight-blue" style="text-align: right;">123* (Offset = 19968)</td>
<td class="highlight-blue">45 (Offset = 20480)</td>
...
1.  Starting at an archive offset of 0 and 25MiB left. This is larger than 18MiB so the jump is used by simply casting the first entry of the line to a uint16\_t stored in a uint32\_t and then bit shifted left by 9.
1.  The offset into the archive is now 5632 and the file offset is now 25MiB - 8 \* 2MiB = 9MiB.
    *  Note that all the block lines are aligned to 512-bytes, so a block that has 34 compressed bytes still takes up 512 bytes in the archive.
1.  Since the 9MiB is less than the 18MiB limit, the blocks are individually inspected. While the offset is larger than 2MiB, add the compressed size (rounded up to multiples of 512) to the archive offset and subtract 2MiB from the offset. This means four blocks will be read. This results in an offset of 5632 (from step 2) + 2048 (rounded up from 2001) + 512 (rounded up from 55) + 1024 (rounded up from 789) + 512 (rounded up from 221) = 9728 bytes.
1.  The remainder is 1MiB, which will need to be used after decompressing at offset 9728 into the decompressed file to copy only the second half of the block(1MiB) to the final output.

Next is figuring till where the final block to read is. The total amount of uncompressed data to read is 14MiB.

1.  Starting at the block that was found for the offset, there was 1MiB read which leaves 13MiB more to read from the following blocks.
1.  13MiB is larger than what remains in this block group so continue to read forward by starting at the next block. This results in an archive offset of 10240 (offset of the next block) + 512 (rounded up from 68) + 1024 (rounded up from 514) + 512 (rounded up from 18) = 12288 and 13MiB - 3 \* 2MiB = 7MiB left to read.
1.  At this point there is less than 18MiB left in the file so there's no jump at the start of this block group and reading can continue
    *  archive offset 12228 + 6656 (rounded up from 6166) = 18944 and 5MiB left.
    *  archive offset 18944 + 512 (rounded up from 364) = 19456 bytes and 3MiB left.
    *  archive offset 19456 + 512 (rounded up from 44) = 19968 bytes and 1MiB left.
2.  There is now less than a 2 MiB left so the next block (123) will need to be read but only 1MiB from it will be copied to the output buffer. This puts the end at offset 20480 after rounding up the final size.

The range to read will be from the archive start at 9728 and end at 20480. A total of 8 blocks need to be decompressed, which means at total 16MiB will be decompressed of which 2 \* 1 MiB is unused matching the total requested size of 14MiB. After decompression the first 1MiB of the first block is skipped and the last 1MiB of the final block is ignored.
